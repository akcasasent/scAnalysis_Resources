...
# this should list all the names of the different samples. 
#all_sam_names_list <-  list.files(general_path)
#sel_sample_info <- sample_info[which(sample_info$Moffit == "Classical"), ]
sel_sample_info <- sample_info[which(sample_info$Procedure == "Resection"), ]
#sel_sample_info <- sel_sample_info[which(sel_sample_info$Treatment == "Naïve"), ]

#selected names 
#sam_names_list <- sel_sample_info$Sample
...
VlnPlot(sample_seurat, features = c("nFeature_RNA","nCount_RNA","percent.mt","percent.rb"),ncol = 4,pt.size = 0.1) & 
  ...
  # remove the those with 
  # <15% of transcripts coming from mitochondrial genes
  ...
  sample_seurat_sub@meta.data[["Procedure"]] <- sel_sample_info$Procedure[which(sel_sample_info$Sample==sample_name)]
  sample_seurat_sub@meta.data[["Stage"]] <- sel_sample_info$Stage[which(sel_sample_info$Sample==sample_name)]
  sample_seurat_sub@meta.data[["Moffit"]] <- sel_sample_info$Moffit[which(sel_sample_info$Sample==sample_name)]
...
pdac.merged <- merge(get(assign_name_list[1]), y = sapply(assign_name_list[-1],function(x) get(x)),
                       ...
                       treatment = sel_sample_info$Treatment,procedure = sel_sample_info$Procedure, 
                       moffit = sel_sample_info$Moffit, stage = sel_sample_info$Stage,
                       project = "PDAC")
...
# this ste migth take a while but it should be printing to your screen
